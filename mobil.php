...
/* Ambil merk unik */
$merkQuery = $conn->query("SELECT DISTINCT merk FROM mobil ORDER BY merk ASC");
            ...
            <?php if ($result->num_rows > 0): ?>
                <?php while ($row = $result->fetch_assoc()): ?>
                    ...
                    <div class="car-card" data-brand="<?= strtolower($row['merk']) ?>">
                        ...
                        <h3><?= htmlspecialchars($row['merk']) ?>         <?= htmlspecialchars($row['nama_mobil']) ?></h3>

                        <p class="price">
                            Rp <?= htmlspecialchars($row['harga_sewa_harian']), 0, ',', '.'; ?>
                        </p>
